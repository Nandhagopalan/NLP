    ...
    def __init__(self, vocab_type):
        self.VOCAB_DICT = {
            'bc5cdr': ('<PAD>', 'B-Chemical', 'O', 'B-Disease' , 'I-Disease', 'I-Chemical',
                        'E-Disease','E-Chemical','S-Chemical','S-Disease'),
        ...
        self.tokenizer = BertTokenizer(vocab_file=config.VOCAB_FILE, do_lower_case=False)
...
class NerTestDataset(data.Dataset):
    def __init__(self, path, vocab_type):
        ...
        for entry in instances:
            ...
            tags = ["<PAD>"]*len(words)
            sents.append(["[CLS]"] + words + ["[SEP]"])
            tags_li.append(["<PAD>"] + ["<PAD>"]*len(words) + ["<PAD>"])
        self.sents, self.tags_li = sents, tags_li
